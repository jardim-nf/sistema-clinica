...
const SidebarItem = ({ icon: Icon, text, to, active, onClick }) => {
    // CLASSE ATIVA MELHORADA: fundo azul escuro, texto branco, e ícone colorido (usando currentColor)
    const activeClasses = 'bg-blue-600 text-white shadow-md';
    // CLASSE INATIVA: texto escuro, hover sutil
    ...
    return (
        ...
        >
            {/* Ícone usa a cor do texto ('currentColor') para ficar branco quando ativo */}
...
export default function Layout({ children }) {
    ...
    // DEFINIÇÃO DAS PERMISSÕES E ROTAS
    const allMenuItems = [
        // ITENS GERAIS
        ...
        // NOVOS ITENS PARA O SUPER ADMIN
    ...
    // Exibe o nome do papel para o Perfil
    ...
    // Se estiver carregando (verificando o login), mostra a tela de carregamento.
    if (isLoading) {
        ...
        <div className="flex h-screen w-full bg-slate-50 font-sans">
            ...
            <aside className={`fixed top-0 left-0 z-30 h-full w-64 bg-white border-r border-slate-200 flex flex-col transition-transform duration-300 ease-in-out ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
                <div className="p-6 h-16 flex items-center justify-between border-b border-slate-100">
                    {/* MELHORIA DO LOGO: Font mais forte e cor consistente */}
                    <h1 className="text-xl font-extrabold text-blue-600 flex items-center gap-2">
                        <span className="bg-blue-600 text-white px-2 py-0.5 rounded text-sm font-bold">SC</span> SysClin
                    </h1>
            ...
            <div className="flex-1 flex flex-col min-h-screen md:pl-64 transition-all duration-300">
                <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-10 shadow-sm">
                    <div className="flex items-center gap-3">
                        ...
                        {/* CABEÇALHO MAIS FORTE: Garantindo contraste e peso */}
                        <h2 className="text-xl font-extrabold text-slate-800">{currentTitle}</h2>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold border border-blue-200 text-sm md:text-base">SC</div>
                    </div>
                </header>

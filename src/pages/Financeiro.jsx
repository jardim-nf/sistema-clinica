...
import { 
  ...
  FileText, Loader2, Trash2, Download, Eye, EyeOff, PieChart,
  ArrowUpRight, ArrowDownRight, Wallet, BarChart3, RefreshCw
} from 'lucide-react';
...
export default function Financeiro() {
  ...
  const handleExcluir = async (item) => {
    if (item.origem === 'agenda') {
      showToast("Para excluir receitas, vá na Agenda.", "warning");
      return;
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm text-slate-500">
              <span className="flex items-center gap-1"><Calendar size={14} /> {formatDate(item.data)}</span>
              {item.pessoa !== '-' && <span className="flex items-center gap-1 truncate max-w-[150px]"><FileText size={14} /> {item.pessoa}</span>}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 pl-2">
          <div className="text-right">
            ...
            <div className="flex items-center justify-end gap-1 text-xs text-slate-400">
              <Clock size={12} /> <span>{item.hora}</span>
            </div>
          </div>
          {item.origem === 'despesa' && (
            <button onClick={() => handleExcluir(item)} className="hidden md:block opacity-0 group-hover:opacity-100 p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
              <Trash2 size={16} />
            </button>
          )}
        </div>
  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-emerald-50 p-4 md:p-6 lg:p-8">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
            <div>
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold text-slate-800 tracking-tight">Gestão Financeira</h1>
                  <p className="text-slate-500 mt-2">Controle completo de receitas e despesas</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-4 justify-between items-center">
              ...
              <div className="flex flex-wrap gap-3 w-full lg:w-auto justify-end">
                ...
                <div className="flex gap-2">
                    <button onClick={() => setViewType(viewType === 'list' ? 'chart' : 'list')} className="p-3 bg-slate-100 text-slate-600 rounded-xl hover:bg-slate-200 transition-all">
                        {viewType === 'list' ? <BarChart3 size={20} /> : <FileText size={20} />}
                    </button>
                    <button onClick={() => setShowDetails(!showDetails)} className="p-3 bg-slate-100 text-slate-600 rounded-xl hover:bg-slate-200 transition-all">
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={dadosGraficoMensal}>
                    ...
                    <Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid #e2e8f0' }} formatter={(value) => [formatMoney(value), '']} />
                    <Line type="monotone" dataKey="receitas" stroke="#10b981" strokeWidth={3} dot={{ r: 4, strokeWidth: 2, stroke: '#ffffff' }} />
                    <Line type="monotone" dataKey="despesas" stroke="#ef4444" strokeWidth={3} dot={{ r: 4, strokeWidth: 2, stroke: '#ffffff' }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
              ...
              <div className="h-80 flex items-center justify-center">
                {pieData.length > 0 ? (
                  ...
                  <div className="text-center text-slate-400"><PieChart size={48} className="mx-auto mb-4 opacity-20" /><p>Sem dados</p></div>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-100">
            <div className="flex justify-between items-center">
              <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2"><FileText size={20} className="text-slate-600" /> Últimas Transações</h3>
              <span className="text-sm text-slate-500">{transacoesFiltradas.length} encontradas</span>
            </div>
          </div>
      ...
      <ModalDespesa isOpen={modalOpen} onClose={() => setModalOpen(false)} onSave={handleSalvarDespesa} />
    </div>
  );
}